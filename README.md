...
## 与 kais-gold-remount（KGR）的关系

KST 在金标工程三方分工中扮演**「造尺子」**角色（考古学 / 归纳），
与 KGR（kais-gold-remount，金标正向复刻工厂 / 复刻工程 / 演绎）互为上下游：

| # | 维度 | KST（本仓） | KGR |
|---|---|---|---|
| 1 | 认识的箭头 | 归纳：原片→抽象描述（求「是什么」） | 演绎：描述→再生产输入（求「能不能做出来」） |
| 2 | 逆向终点 | 描述性资产：P00-P07 JSON + shot-list + 定级标签（答案纸） | 执行性资产：re-recipe = KMC 生成器原生输入（配方卡） |
| 3 | 验收深度 | 描述对不对（vs 帧证/ASR 锚定）+ 金标自证可信 | 输入能不能再生产出原片（真实引擎往返 + 盲测） |
| 4 | 对 LLM 断言的立场 | 证据约束（断言必须挂帧/ASR 锚，防编造） | 闭合倒逼（写得不准引擎就复现不出，骗不过物理） |
| 5 | score_qc 语义 | 金标验证工具（判金标分镜可实现性） | 闭合验收工具（判 re-recipe 再生产能力） |

**数据流**：

```
KST ──帧证/ASR/时间轴/evidence packs（只读消费）──▶ KGR（S3 配方逆向 → S4 闭合 → S6 全链重放）
KGR ──闭合天花板（引擎边界图，如 H3 时相域 prompt 不可达）──▶ KST render-back 提前止损
```

**一条铁律：描述对了 ≠ 能复现。** KST 的 T1-T4 证明「逆向描述忠实于原片」；
KGR 的 S4/S6 证明「从描述重建的输入包能重演原片」。金标只有同时过两关
（KST anchored 定级 + KGR 节点闭合）才有资格当 KMC 迭代循环二的对比目标——
**两仓联合签发，缺一不可**。

**边界纪律**：KGR 只读消费本仓机械层证据，不回写金标；金标可信度归 KST 管，
管线能力改进归 kais-kmc-iteration 管。三方全景：KST 造尺子 → KGR 复刻 →
kmc-iteration 改工厂。完整定义见 KGR 仓 README「附：与 KST 的区别与联系」。

...
- [x] 19-03-PLAN.md — run_pipeline 无编号 pre-step 5.6 挂载（--vision-seq/--no-vision-seq/--no-ear，plain-label banner）+ wiring 四件套测试 + SC1/SC4 集成证据（sandbox 填充 diff + live prompts.json sha 不变负测试 + 秒级重跑实测）
...
- [x] 21-03-PLAN.md — 批后 19 镜全量双信号 → 分布材料（分位数/三桶/τ 预演）+ 校准报告草稿 → Kai τ_sim 裁决 + 抽检 5 镜 checkpoint → --apply-verdict + PROJECT.md Key Decisions + rejected 分桶审计 + 幂等证明
...
- [x] 22-02-PLAN.md — HITL 回写 + dataset 导出：roundtrip-edits.schema.json + apply_edits.py（confirmed-only + source:human + 重放幂等）+ export_dataset.py（帧直拷优先 + manifest + accepted/rejected 分清单）+ 单测×2

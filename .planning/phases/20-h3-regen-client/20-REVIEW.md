...
**Outcome (fix):** FIXED (two-layer validation, (a)+(b) combined) — `write_roundtrip_sidecar` now returns `list[str]`: layer ① validates this batch's `entries` in isolation → `sys.exit` on error (fail-loud, unchanged contract); layer ② validates the merged payload and, on error, attributes offending entries to `shot_id`s via error `absolute_path`, emits a per-shot `[roundtrip]` str warning, backs the pre-existing file up to `roundtrip.json.bak-<ts>`, drops those entries, re-validates, and writes the rest — no retry deadlock. Unattributable (top-level) errors still fail loud. `main()` extends `pending_warnings` with the returned warnings before the flush (they would be eaten if written directly, due to the strip-previous-round semantics). Regression anchors: `test_sidecar_preexisting_bad_entry_skipped_not_deadlocked`, `test_sidecar_bad_preexisting_entry_e2e_warning_flush`; `test_sidecar_refuses_invalid_path` still pins layer ① fail-loud.

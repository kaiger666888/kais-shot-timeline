...
| 18-03-T1 | 18-03 | 2 | RT-03 | T-18-09/10/11 | AF-01 禁语 0 匹配；enum 与 schema 逐字一致；阈值只散文；XSS 义务预告 | grep + cli | Task `<verify>` python：§1 14 计数 + Changelog 1.3 + Round-trip 节 + §10 关键词 + 6 禁语 0 匹配 + fixture 一致最小片段 + validate.py 绿 | ✅（待编辑） | ⬜ pending |
| 18-03-T2 | 18-03 | 2 | RT-03 | T-18-09 | README 诚实 delta 记录 + AF-01 | grep | Task `<verify>` python：v1.3 section + 14 计数 + footer + 6 禁语 0 匹配 | ✅（待编辑） | ⬜ pending |
| 18-03-T3 | 18-03 | 2 | RT-03 | T-18-10/11 | 人类审阅 = RT-03「一次人类审阅通过」验收 | manual-only（见下表） | —（ROADMAP SC#5 明文要求） | ✅ | ⬜ pending |

...
> **⚠️ v1.3 盲测终审修正 (Kai 2026-09-01, shot89 A2V)**：量化门槛全过 ≠ 盲测可过，**必须收口进盲测终审页才定收敛**。实测教训：拼接版(stitch 7×100)聚合 QC 最优(SSIM 0.579/SEM 0.855 双门过)，但 Kai 盲测 3×「明显差异」——7 段异 seed 拼接的**接缝处身形/光照微跳变是感知杀手，24 帧配对 SSIM/SEM 看不见接缝**；同批单臂 lot100(量化唯一双门过)反而仅「轻微差异」。第二例「量化↑≠感知↑」实锤(第一例=dHash 13.8 压线 vs SSIM 0.479)。盲测识别原片 4/5(预注册口径=仍可分辨；n=5 统计上属方向性信号)。处置：单臂 lot100 锁定为锚定版，拼接降级为方法论实验；残差(身形微差+躬身时相偏早 0.5-1s)定性引擎边界不烧 prompt。**聚合分数冠军≠盲测冠军，任何「收敛/达标」表述以盲测终审为准。**

...
用法: python3 agreement_report.py <relabel_run.json> [--sealed <golden_answers_sealed.json>]
默认 sealed 路径 = /data/workspace/kais-shot-timeline/gsrt/golden_answers_sealed.json (ep01 实例)
输出: <同目录>/agreement_report_<run名>.json
维度: emotion 同义聚类三档 / felt_intent role-signal 集合 / shot_type 别名 exact
"""
...
SHOT_ALIAS = {'特写': ['大特写','特写'], '近景': ['中近景','近景'], '中景': ['中景'],
              '中全景': ['中全景','中远景'], '全景': ['全景','全景镜头'], '远景': ['远景','大远景','外景']}

DEFAULT_SEALED = '/data/workspace/kais-shot-timeline/gsrt/golden_answers_sealed.json'
...
def emo_match(g, r):
    ...
    if gb.startswith('other') or rb.startswith('other'):
        return 'partial' if len(set(g) & set(r)) >= 2 else 'off'
    return 'off'
...
def shot_type_match(g, r):
    g, r = norm(g), norm(r)
    if not g or not r: return 'missing'
    for k, vals in SHOT_ALIAS.items():
        if (k in g or g in vals) and (k in r or r in vals): return 'exact'
    return 'off'

def _interpret(s):
    e = s['emotion']['agreement_exact_plus_partial_pct'] or 0
    i = s['felt_intent']['agreement_exact_plus_partial_pct'] or 0
    if e >= 70 and i >= 70: return ['T3高一致: 金标推断层可信, 零点有效']
    if e < 40 and i < 40: return ['T3低一致: 金标推断层单采样噪声嫌疑大, 建议多标注集成或降级为参考']
    return ['T3中等: 需分歧字段抽样视频终审定谳']

def main(run_path, sealed_path):
    answers = {a['shot_id']: a for a in json.load(open(sealed_path))}
    ...
    for dim in ('emotion', 'felt_intent', 'shot_type'):
        ...
        summary[dim] = {
            'exact': c.get('exact', 0), 'partial': c.get('partial', 0),
            'off': c.get('off', 0), 'missing': c.get('missing', 0),
    ...
    out = {'run': run_path, 'sealed': sealed_path,
           'judged_at': datetime.now(timezone.utc).isoformat(),
           'n_shots': len(rows), 'summary': summary,
           'interpretation': _interpret(summary), 'rows': rows}
    outp = re.sub(r'[^0-9A-Za-z_-]', '', run_path.rsplit('/', 1)[-1])
    outp = outp.replace('.json', '') or 'run'
    dest = f"{run_path.rsplit('/', 1)[0]}/agreement_report_{outp}.json"
    json.dump(out, open(dest, 'w'), ensure_ascii=False, indent=1)
    print(json.dumps({'output': dest, 'summary': summary, 'interpretation': out['interpretation']}, ensure_ascii=False, indent=1))

if __name__ == '__main__':
    run = sys.argv[1]
    sealed = sys.argv[sys.argv.index('--sealed')+1] if '--sealed' in sys.argv else DEFAULT_SEALED
    main(run, sealed)

...
"""
import json, subprocess, time, sys, os, glob
...
def extract(sid):
    ...
    r = subprocess.run(["docker","cp",f"comfyui-primary:/root/ComfyUI/output/{fname}",dst],
                       capture_output=True, text=True)
...
# ─── Phase A: 提交/守望决策 ───
watching = []   # (sid) 无 prompt_id, 盯容器产物
for s in manifest:
    ...
    # pending / submit_failed(64): 64 不重提(有5个僵尸等待者会渲染它), 新镜头单次提交
    if sid == 64 and j.get("status") in ("submit_failed", "pending"):
        mark(sid, "watching")
        watching.append(sid)
        continue
...
while True:
    ...
    for s in manifest:
        ...
        if st == "done":
            continue
        remaining += 1
        # 产物先到先收 (适用于 rendering/watching 所有状态)
        ok, path = extract(sid)
        if ok:
            mark(sid, "done", file=path)
            remaining -= 1
            continue
        # rendering: KAP 状态轮询兜底

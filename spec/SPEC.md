...
| `generator.warnings` | array\<string \| {code, detail}\> | — (v1.1;items 加宽 v1.3) | 非致命警告列表(如运镜路由不可达 → prompts 空字段降级)。**v1.3(Phase 18)起 items 加宽为双形**:legacy 纯文本 string,或结构化 `{code, detail}` —— `code` 是 RT-04 closed enum(`comfyui_unreachable` / `vram_insufficient` / `scorer_model_missing`,roundtrip 缺席三因由,机器可 grep),`detail` optional string(逐字对齐 `asset.schema.json#generator.warnings.items`;见 §4 Changelog `1.3`)。仅在非空时 emit;老资产及干净运行缺省仍合法(graceful-degrade)。Producer: Plan 02 `analysis/call_shot_analysis.py` 在路由失败时写 `route_cache/warnings.json` sidecar,由 `scripts/export_asset.py:main` best-effort 读入合并进 `generator.warnings`。Operator-facing failure reasons only(exception class + message, route status codes)— no PII, no auth tokens, no body payloads |
